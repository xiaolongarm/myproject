//
//  DataAq4GDataFlowTableViewController.m
//  CMCCMarketing
//
...
#import "DataAq4GDataFlowTableViewCell.h"

@interface DataAq4GDataFlowTableViewController ()<PreferentialPurchaseSelectDateTimeViewControllerDelegate,MBProgressHUDDelegate>
{
...
@implementation DataAq4GDataFlowTableViewController
...
-(void)InitControl{
    ...
    packageNameArray=[NSMutableArray array];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString * identifier = @"DataAq4GDataFlowTableViewCell";
    DataAq4GDataFlowTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle]loadNibNamed:@"DataAq4GDataFlowTableViewCell" owner:nil options:nil]lastObject];
    }
    //去掉选中效果
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    
    cell.customPhoneNumber.keyboardType=UIKeyboardTypeNumberPad;
    cell.dataFlowType.keyboardType=UIKeyboardTypeNumberPad;
...
-(void)goSendData{
    ...
    for (int i = 0; i < [tableviewArray count]; i++) {
        ...
        DataAq4GDataFlowTableViewCell *scell = (DataAq4GDataFlowTableViewCell*)[self.tableView cellForRowAtIndexPath:indexPath];
        ...
        //宽带类型
        NSString *dataFlowTypeString= scell.dataFlowType.text;
        [packageNameArray addObject:dataFlowTypeString];
        //金额**************************
    ...
    //办理套餐名称string
    NSString *packageString= [packageNameArray componentsJoinedByString:@","];
    //入单金额
    NSString *mountString= [keyAmountArray componentsJoinedByString:@","];
    //办理时间string
    NSString *timeString = [dealTimeArray componentsJoinedByString:@","];
    //***********************
    //提交参数
    //  宽带业务
    //    shaoyang\v1_2\datacollect\databroadband
    //    必传参数
    ...
    //    broadband_msisdn办理手机号码(手机端录入有多个时用,号分隔)
    //    broadband_name 宽带类型(手机端录入有多个时用,号分隔)
    //    broadband_money 金额(手机端录入有多个时用,号分隔)
    //    broadband_date 办理时间(手机端录入有多个时用,号分隔)
    //    返回参数
    //    flag 返回成功结果
    ...
    [bDict setObject:cardString forKey:@"broadband_msisdn"];
    [bDict setObject:packageString forKey:@"broadband_name"];
    [bDict setObject:mountString forKey:@"broadband_money"];
    [bDict setObject:timeString forKey:@"broadband_date"];
    ...
    [NetworkHandling sendPackageWithUrl:@"datacollect/databroadband" sendBody:bDict processHandler:^(NSDictionary *result, NSError *error) {
...
-(void)showSuccessMessage:(NSString*)infomation{
    ...
    [packageNameArray removeAllObjects];
