//
//  DataAq4GPackageTableViewController.m
//  CMCCMarketing
//
...
#import "DataAq4GPackageTableViewCell.h"


@interface DataAq4GPackageTableViewController ()<PreferentialPurchaseSelectDateTimeViewControllerDelegate,MBProgressHUDDelegate>
{
...
@implementation DataAq4GPackageTableViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString * identifier = @"DataAq4GPackageTableViewCell";
    DataAq4GPackageTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle]loadNibNamed:@"DataAq4GPackageTableViewCell" owner:nil options:nil]lastObject];
    }
    //去掉选中效果
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    
    
    //cell.customPhoneNumber.placeholder=@"客户号码";
...
-(void)goSendData{
    ...
    for (int i = 0; i < [tableviewArray count]; i++) {
        ...
        DataAq4GPackageTableViewCell *scell = (DataAq4GPackageTableViewCell*)[self.tableView cellForRowAtIndexPath:indexPath];
        ...
        //办理套餐名称
        NSString *packageNameString= scell.packageName.text;
        [packageNameArray addObject:packageNameString];
        //办理时间
   ...
   //客户号码string
    ...
    //提交参数
    //    4g套餐
    //    shaoyang\v1_2\datacollect\data4gpackage
    //    必传参数
    ...
    //    package_4g_msisdn 办理手机号码(手机端录入有多个时用,号分隔)
    //    package_4g_name 套餐名称(手机端录入有多个时用,号分隔)
    //    package_4g_date 办理时间(手机端录入有多个时用,号分隔)
    //    返回参数
    //    flag 返回成功结果
    ...
    [bDict setObject:cardString forKey:@"package_4g_msisdn"];
    [bDict setObject:packageString forKey:@"package_4g_name"];
    [bDict setObject:timeString forKey:@"package_4g_date"];
    //***********************
    ...
    [NetworkHandling sendPackageWithUrl:@"datacollect/data4gpackage" sendBody:bDict processHandler:^(NSDictionary *result, NSError *error) {
