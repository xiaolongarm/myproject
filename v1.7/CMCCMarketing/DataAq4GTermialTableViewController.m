//
//  DataAq4GTermialTableViewController.m
//  CMCCMarketing
//
...
#import "DataAq4GTermialTableViewCell.h"
#import "PreferentialPurchaseSelectDateTimeViewController.h"

@interface DataAq4GTermialTableViewController ()<PreferentialPurchaseSelectDateTimeViewControllerDelegate,MBProgressHUDDelegate>
{
    NSMutableArray *tableviewArray;
    NSMutableArray *customNumberArray;
    NSMutableArray *termialTypeArray;
    NSMutableArray *termialClassifyArray;
    NSMutableArray *keyAmountArray;
...
@implementation DataAq4GTermialTableViewController
...
-(void)InitControl{
    ...
    termialTypeArray=[NSMutableArray array];
    termialClassifyArray=[NSMutableArray array];
    keyAmountArray=[NSMutableArray array];
...
//        static NSString * identifier = @"DataAq4GTermialTableViewCell";
//    DataAq4GTermialTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:identifier];
//    if (cell == nil) {
//        cell = [[[NSBundle mainBundle]loadNibNamed:@"DataAq4GTermialTableViewCell" owner:nil options:nil]lastObject];
//    }
    DataAq4GTermialTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"DataAq4GTermialTableViewCell" forIndexPath:indexPath];
    //去掉选中效果
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.customPhoneNumber.keyboardType=UIKeyboardTypeNumberPad;
    cell.termialType.keyboardType=UIKeyboardTypeNumberPad;
...
-(void)goSendData{
    ...
    for (int i = 0; i < [tableviewArray count]; i++) {
        ...
        DataAq4GTermialTableViewCell *scell = (DataAq4GTermialTableViewCell*)[self.tableView cellForRowAtIndexPath:indexPath];
        ...
        //办理终端名称
        NSString *termialTypeString= scell.termialType.text;
        [termialTypeArray addObject:termialTypeString];
        //办理终端类型
        NSString *termialClassifyString= scell.termialClassify.text;
        [termialClassifyArray addObject:termialClassifyString];
        //录入金额
        NSString *keyAmountString= scell.keyAmount.text;
        if ([keyAmountString isEqualToString:@""]) {
            keyAmountString= @"0";
        }

        [keyAmountArray addObject:keyAmountString];
        //办理时间
    ...
    //办理终端名称string
    NSString *typeString= [termialTypeArray componentsJoinedByString:@","];
    //办理终端类型
     NSString *classifyString= [termialClassifyArray componentsJoinedByString:@","];
    //录入金额
     NSString *amountString= [keyAmountArray componentsJoinedByString:@","];
    //办理时间string
    NSString *timeString = [changeCardTimeArray componentsJoinedByString:@","];
    //提交参数
    //    4g终端
    //    shaoyang\v1_2\datacollect\data4gterm
    //    必传参数
    ...
    //    term_4g_msisdn 办理手机号码(手机端录入有多个时用,号分隔)
    //    term_4g_name 终端型号(手机端录入有多个时用,号分隔)
    //    term_4g_type 终端类型(手机端录入有多个时用,号分隔)
    //    term_4g_money 金额(手机端录入有多个时用,号分隔)
    //    term_4g_date 办理时间(手机端录入有多个时用,号分隔)
    //    返回参数
    //    flag 返回成功结果
    ...
    [bDict setObject:cardString forKey:@"term_4g_msisdn"];
    [bDict setObject:typeString forKey:@"term_4g_name"];
    [bDict setObject:classifyString forKey:@"term_4g_type"];
    [bDict setObject:amountString forKey:@"term_4g_money"];
    [bDict setObject:timeString forKey:@"term_4g_date"];
    //***********************
    ...
    [NetworkHandling sendPackageWithUrl:@"datacollect/data4gterm" sendBody:bDict processHandler:^(NSDictionary *result, NSError *error) {
...
-(void)showSuccessMessage:(NSString*)infomation{
    ...
    [termialTypeArray removeAllObjects];
    [termialClassifyArray removeAllObjects];
    [keyAmountArray removeAllObjects];
    ...
    for (int i = 0; i < [tableviewArray count]; i++) {
        ...
        NSLog(@"%lu",(unsigned long)row);
        scell.customPhoneNumber.text=@"";
        scell.termialType.text=@"";
        scell.termialClassify.text=@"";
        scell.keyAmount.text=@"";
        scell.dealTimeButton.titleLabel.text=@"点击选择";
    }
     [self.tableView reloadData];
